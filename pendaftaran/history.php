...
<head>
  ...
  <title>Humanika | History Pembayaran</title>
  ...
  <nav class=" navbar navbar-default" role="navigation">
		...
		<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-">
                <ul class="nav navbar-nav navbar-right">
					...
					<li><a href="daftar-peserta.php?keg=<?php echo $ambil?>&tahun=<?php echo $tahun?>"><i class="fa fa-users"></i>  Daftar Peserta</a></li>
					...
					<li>
						...
						<ul class="dropdown-menu dropdown-menu-usermenu pull-right">
							...
							<li class="active"><a href="history.php?id=<?php echo $_SESSION['id_peserta']; ?>&keg=<?php echo $ambil?>&tahun=<?php echo $tahun ?>"><i class="fa fa-money"></i> History Pembayaran</a></li>
  ...
  <!--body wrapper start-->
							...
							<?php
								...
								foreach ($hasil as $data) {
									if($data->tgl_byr_awal<=$tgl && $data->tgl_byr_akhir>=$tgl){ 
													...
													if($_SESSION['jenis']==1){echo"<a href='se/konfirmasi_pembayaran.php?keg=SE&nim=".$_SESSION['nim']."&peserta=".$_SESSION['id_peserta']."&tahun=$thn'>
													<button class='btn btn-primary btn-lg' type='button' >KONFIRMASI</button></a>";}
													else if($_SESSION['jenis']==2){echo"<a href='ldkm/konfirmasi_pembayaran.php?keg=SE&nim=".$_SESSION['nim']."&peserta=".$_SESSION['id_peserta']."&tahun=$thn'>
													<button class='btn btn-primary btn-lg' type='button' >KONFIRMASI</button></a>";}
													echo"</center>";}
													else if($st=='Lunas'){
													echo"	
													...
													<center><h3>".DateToIndo($data->tgl_byr_awal)." - ". DateToIndo($data->tgl_byr_akhir)."</h3>
													<h2>Pembayaran Sudah Ditutup</h2></center>
													";}
									else if($data->tgl_byr_awal>$tgl && $data->tgl_byr_akhir>$tgl){ echo " 
													<center><h3>".DateToIndo($data->tgl_byr_awal)." - ". DateToIndo($data->tgl_byr_akhir)."</h3>
													<h2>Pembayaran Masih Belum Dibuka</h2></center>
													";}				
        ...
        <div class="col-md-8">
		  <?php 
			...
			$rincian=$clientbyr->RincianBiaya($kode, $ambil); 
		?>
            ...
            <div class="panel">
				<div class="panel-body">
					...
					<tr >
						...
						<tr class='gradeX'>
							...
							<td align='center' width='150px'>Rp. $data->jumlah,-</td>
							<td align='center'>".DateToIndo($data->tgl_pembayaran)."</td> 
						</tr>";  $jumlah=$jumlah+$data->jumlah; $sisa=$data->biaya-$jumlah; $stat=$data->status_bayar;
						} 
						?>
						</tbody>
				</table> 
				...
				<?php 
							...
							echo"<div class='alert alert-success fade in'>
												...
												<strong>Penting! </strong><span> Total Pembayaran Anda : $jumlah.</br>
												Sisa Pembayaran Anda : $sisa</br>
							...
							echo"<div class='alert alert-success fade in'>
												...
												<strong>Penting! </strong><span> Total Pembayaran Anda : $jumlah.</br>
												Status pembayaran anda SUDAH LUNAS </br>";
												if($ambil=='SE'){
												echo "<a href='se/kwitansi_pembayaran.php?keg=$ambil&peserta=$_SESSION[id_peserta]&tahun=$tahun' target='_blank'><button class='btn btn-primary' type='button' ><i class='fa fa-print'></i> Cetak Kwitansi</button></a>";}
												else if($ambil=='LDKM'){echo "<a href='ldkm/kwitansi_pembayaran.php?keg=$ambil&peserta=$_SESSION[id_peserta]&tahun=$tahun' target='_blank'><button class='btn btn-primary' type='button' ><i class='fa fa-print'></i> Cetak Kwitansi</button></a>";}
												echo"</span>
											</div>";
							}
							?>
